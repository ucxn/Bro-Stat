...
async function rSD() {
    ...
    try {
      let stk = location.pathname.match(/stok=([a-zA-Z0-9]+)/i)?.[1] || location.href.match(/stok=([a-zA-Z0-9]+)/i)?.[1];
      if (!stk) return;
      const ts = Date.now();

      // TP-Link 标准 JSON-RPC POST 请求
      let res = await fetch(`/stok=${stk}/ds`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json; charset=UTF-8' },
          body: JSON.stringify({ network: { name: ["wan_status"] }, hosts_info: { table: "online_host" }, method: "get" })
      });
      
      if (!res.ok) return;
      let dD = await res.json();
      if (dD.error_code !== 0) return;

      let cWU = (+dD.network?.wan_status?.up_speed || 0) * 8;
      let cWD = (+dD.network?.wan_status?.down_speed || 0) * 8;
      
      S.hasW2 = !1; 
      let cSU = 0, cSD = 0, cI = Object.create(null);
      (dD.hosts_info?.online_host || []).forEach(obj => {
      ...
      for (let n of gI) {
        ...
        if (dI && inf) {
          ...
          if (!bx || bx.querySelector('.t-row')) {
            ...
            bx.innerHTML = `<div class="zte-thin-bar" style="flex:1; margin:0;"><div class="zte-thin-bar-inner up"></div></div><span class="v-pct c-up" style="font-size:12px; font-weight:bold; font-family:Consolas; width:45px; text-align:right;"></span>`;
            inf.appendChild(bx);
            cache.upBox = bx;
          }
          ...
          if (!dBx || dBx.querySelector('.t-row')) {
            ...
            dBx.innerHTML = `<div class="zte-thin-bar" style="flex:1; margin:0;"><div class="zte-thin-bar-inner down"></div></div><span class="v-pct c-down" style="font-size:12px; font-weight:bold; font-family:Consolas; width:45px; text-align:right;"></span>`;
            inf.appendChild(dBx);
            cache.dBox = dBx;
          }
  ...
  async function eBET(fW = !0) {
    ...
    try {
      ...
      if (fW) {
        ...
        } catch(e) {console.warn("[哥哥科技] 主动拉取WAN口数据异常", e);}
        wST = performance.now();
      }
      let lT = "", lF = "";
